...
After you have filled out `banners.json` to your project's specifications, there are two scaffolding processes you can run.
...
gulp re-scaffold
```
This task will **delete all the files** from `resources/html/pages`, `resources/scss/pages`, `resources/js/pages`, and `resources/img/pages`, and then re-scaffold your project from `banners.json`. Be careful before running this command and double-check you aren't overwriting something you need.
...
A quick guide to the various Gulp tasks in this framework. If you do not have Gulp globally installed, every task covered here can also be accessed by running `npm run <gulp-task>`.
...
```
The link name that you pass in should match a <link-name> that you placed in `banners.json`. The second parameter will be added as a class to the link when rendered. Some classes will be added automatically, but you can optionally add your own here.
...
This framework uses Nunjucks Rendering to create HTML files during it's build process. You can learn more about Nunjucks by reading their documentation <a href="https://mozilla.github.io/nunjucks/">here</a>
